CLASS zcl_ctsw_transport_dpc_ext DEFINITION
  PUBLIC
  INHERITING FROM zcl_ctsw_transport_dpc
  CREATE PUBLIC .
  ...
  PRIVATE SECTION.
    ...
    METHODS sort_and_compress
      CHANGING
        ct_entityset TYPE zcl_ctsw_transport_mpc=>tt_object.

ENDCLASS.



CLASS zcl_ctsw_transport_dpc_ext IMPLEMENTATION.


  METHOD /iwbep/if_mgw_appl_srv_runtime~execute_action.

    DATA: ls_return TYPE zcl_ctsw_transport_mpc=>ts_returncode.

    CASE iv_action_name.
      WHEN 'Release'.
        zcl_ctsw_transport_dao=>release_transport(
          EXPORTING
...
*-----------------------------------------------------------------------
    ...
    DATA: ls_compare_entity    TYPE zcl_ctsw_transport_mpc_ext=>ts_compareversions,
    ...
    DATA(lo_version_comparison) = NEW zcl_ctsw_tr_versions( ).
...
*-----------------------------------------------------------------------
    ...
    DATA: lo_dao    TYPE REF TO zcl_ctsw_transport_dao.
...
*-----------------------------------------------------------------------
    ...
    DATA: lo_dao    TYPE REF TO zcl_ctsw_transport_dao,
          ls_entity TYPE zcl_ctsw_transport_mpc=>ts_import_system.
